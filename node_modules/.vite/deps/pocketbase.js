...
var BaseAuthStore = class {
  ...
  exportToCookie(e2, t2 = i) {
    ...
    const r = { token: this.token, record: this.record ? JSON.parse(JSON.stringify(this.record)) : null };
    let o = cookieSerialize(t2, JSON.stringify(r), e2);
    const a = "undefined" != typeof Blob ? new Blob([o]).size : o.length;
    if (r.record && a > 4096) {
      r.record = { id: (_a = r.record) == null ? void 0 : _a.id, email: (_b = r.record) == null ? void 0 : _b.email };
      const s3 = ["collectionId", "collectionName", "verified"];
      for (const e3 in this.record) s3.includes(e3) && (r.record[e3] = this.record[e3]);
...
var RecordService = class extends CrudService {
  ...
  async authWithPassword(e2, t2, s2) {
    ...
    return n2 = this.authResponse(n2), i2 && this.isSuperusers && function registerAutoRefresh(e3, t3, s3, i3) {
      resetAutoRefresh(e3);
      const n3 = e3.beforeSend, r = e3.authStore.record, o = e3.authStore.onChange((t4, s4) => {
        (!t4 || (s4 == null ? void 0 : s4.id) != (r == null ? void 0 : r.id) || ((s4 == null ? void 0 : s4.collectionId) || (r == null ? void 0 : r.collectionId)) && (s4 == null ? void 0 : s4.collectionId) != (r == null ? void 0 : r.collectionId)) && resetAutoRefresh(e3);
      });
      e3._resetAutoRefresh = function() {
        o(), e3.beforeSend = n3, delete e3._resetAutoRefresh;
      }, e3.beforeSend = async (r2, o2) => {
        var _a;
        const a = e3.authStore.token;
        if ((_a = o2.query) == null ? void 0 : _a.autoRefresh) return n3 ? n3(r2, o2) : { url: r2, sendOptions: o2 };
        ...
        return o2.headers = l, n3 ? n3(r2, o2) : { url: r2, sendOptions: o2 };
      };
...
var SubBatchService = class {
  ...
  prepareRequest(e2, t2) {
    if (normalizeUnknownQueryParams(t2), e2.headers = t2.headers, e2.json = {}, e2.files = {}, void 0 !== t2.query) {
      const s2 = serializeQueryParams(t2.query);
      s2 && (e2.url += (e2.url.includes("?") ? "&" : "?") + s2);
    }
    for (const s2 in t2.body) {
      const i2 = t2.body[s2];
      if (isFile(i2)) e2.files[s2] = e2.files[s2] || [], e2.files[s2].push(i2);
      else if (Array.isArray(i2)) {
        const t3 = [], n2 = [];
        for (const e3 of i2) isFile(e3) ? t3.push(e3) : n2.push(e3);
        if (t3.length > 0 && t3.length == i2.length) {
          e2.files[s2] = e2.files[s2] || [];
          for (let i3 of t3) e2.files[s2].push(i3);
        } else if (e2.json[s2] = n2, t3.length > 0) {
          let i3 = s2;
          s2.startsWith("+") || s2.endsWith("+") || (i3 += "+"), e2.files[i3] = e2.files[i3] || [];
          for (let s3 of t3) e2.files[i3].push(s3);
        }
      } else e2.json[s2] = i2;
    }
  }
