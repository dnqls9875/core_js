...
export function createUserCard({ id, username = 'unknown', email = 'unknown@unknown.com', website = 'unknown.co.kr' }) {
  return `
...
// 2-1 로딩 스피너 태그 생성하는 함수

export function createLoadingSpinner(size = 100, loadingMessage = '유저 정보를 가져오는 중...') {
  return `
...
// 2-2 로딩 스피터 랜더링 하는 함수
export function renderLoadingSpinner(target) {
  insertLast(target, createLoadingSpinner());
}

// 3-1 empty 카드 태그를 생성
export function createEmptySvg(size = 200, errorMessage = '죄송합니다. 데이터가 없습니다.') {
  return `
