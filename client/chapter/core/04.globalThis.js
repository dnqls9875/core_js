...
/* Global This        */
/* ------------------ */
// & var를 사용하지 않고, let,const를 사용하는 이유는 var는 선언하면 전역을 오염시킨다. (전역으로 값이 포함이 된다.)
...
// % window.alert alert이 window객체에 포함이 되어 있기 때문에 window를 생략해줘도 호출이 가능하다.
// % alert('안돼');

// & global, globalThis는 노드에서 전역객체로 사용 가능하다.
...
// & 전역으로 값이 포함되는 건 상당히 위험하다.
// & let,const는 window에 bind가 되지 않는거지 최상위객체는 여전히 window다.
