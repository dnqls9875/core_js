...
receivedEmailAddress = emailAddress === undefined || emailAddress === null ? 'company.io' : emailAddress;

// const ternary = emailAddress === undefined || emailAddress === null ? (receivedEmailAddress = 'user@company.io') : (receivedEmailAddress = emailAddress);
...
console.log(isActive || WIDTH); // 10px  데이터 타입으로 변환해서 truthy값을 찾는다.
console.log(isActive ?? WIDTH); // false  형 변환 없이 정의되어서 false를 반환

console.log('' || WIDTH);
console.log('' ?? WIDTH);

// a || = b a가 false 일 때 b의 값을 a에 할당한다.
// a && = b a가 true 일 때 b의 값을 a에 할당한다.
// a ?? = b a가 undefined, null 일 때 b의 값을 a에 할당한다.
